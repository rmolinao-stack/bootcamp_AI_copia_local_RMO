...
# RMO: Esta demo solo sirve para verificar que el fichero faq.json
# tiene una estructura de lista de diccionarios como se espara.
...
def demo_perfiles() -> None:
    ...
    for perfil in ("junior", "senior", "mentor"):
        # RMO: Crea un nuevo diccionario (concepturalmente un objeto)
        # con la estructura base definida.

        config = deepcopy(ASSISTANT_CONFIG_DEFAULT)
        config["perfil_activo"] = perfil

        # RMO: Crea un diccionario para guardar el estado/contexto
        # de la consulta que vamos a lanzar
        ...
        # procesar_turno(state, " d  ", assistant_config=config)
