"""Generación de embeddings con Gemini — Sprint 8.

Lee output/chunks.json, convierte cada chunk en un vector numérico y guarda
output/embeddings.json. También expone embeddear_consulta() para el retriever (S9).

Requiere GEMINI_API_KEY en .env (ver gemini_auth.py).
"""
...
def cargar_chunks_json() -> list[dict]:
    """Lee la lista de chunks generada por pipeline.py."""
...
def embeddear_textos(client: genai.Client, textos: list[str]) -> list[list[float]]:
    ...
    Un types.Content por texto → un vector por chunk o por pregunta.
    """
    if not textos:
        return []
...
def embeddear_consulta(client: genai.Client, pregunta: str) -> list[float]:
    """Un vector para la pregunta del usuario (mismo modelo que el índice).

    Usado por retriever.py en Sprint 9: la pregunta debe embeddearse con el
    mismo EMBEDDING_MODEL que los chunks indexados en Chroma.
    """
...
def ejecutar_embeddings() -> tuple[list[dict], Path]:
    """Orquesta S8: chunks.json → vectores → embeddings.json."""
    ...
    # Límite de demo para no saturar la API (configurable en config.py)
