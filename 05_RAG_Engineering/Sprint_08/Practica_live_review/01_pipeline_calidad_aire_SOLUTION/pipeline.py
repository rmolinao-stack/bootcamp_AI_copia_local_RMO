"""Orquestación del pipeline de ingesta (Fase 1).

Flujo: load → clean → chunk → guardar chunks.json

load.py implementa la ingesta del CSV; el resto de pasos ya están conectados aquí.
"""
...
def _nombre_fuente(metadata: dict) -> str:
    """Nombre de archivo a partir de metadata['source'] (p. ej. faq_calidad_aire.md)."""
...
def imprimir_resumen_consola(chunks: list[Document], stats: dict) -> None:
    """Muestra conteos por fuente y una muestra de chunks (FAQ/CSV)."""
    print("\n--- Resumen ingesta ---")
    ...
    # Busca un chunk que venga del CSV (metadata tipo meteo_medicion)
    for chunk in chunks:
...
def ejecutar_ingesta() -> tuple[list[Document], Path, dict]:
    """Paso a paso de la Fase 1 hasta generar chunks.json."""
